...
export const AjustesPage = () => {
  ...
  // Importar datos desde CSV de otras apps.
  ...
  const themeOptions = [
    {
      ...
      label: t("ajustes.temaNoche"),
      description: t("ajustes.temaNocheDesc"),
      icon: Moon,
    },
    {
      value: "day" as const,
      label: t("ajustes.temaDia"),
      description: t("ajustes.temaDiaDesc"),
      icon: Sun,
    },
  ];
  ...
  const setWarmupPercents = (raw: string) => {
    ...
    const invalid = tokens.filter(
      (_, i) =>
        !Number.isFinite(parsed[i]) || parsed[i] <= 0 || parsed[i] > 100,
    );
    if (invalid.length > 0) {
      setWarmupError(
        t("ajustes.warmupError", {
      ...
      <div className="space-y-5 p-4 pb-32">
        ...
        <section className="panel-light rounded-2xl p-4">
                ...
                >
                  <span
                    className={`size-6 rounded-full ${PALETTE_SWATCH[value]} ${
                      isActive
                        ? "ring-2 ring-fg/60 ring-offset-1 ring-offset-bg-elevated"
                        : ""
                    }`}
                    aria-hidden
                  />
          ...
          <div className="mt-4 flex items-center justify-between gap-3 border-t border-border/50 pt-3">
            <div className="min-w-0">
              <p className="text-sm font-medium text-fg">
                {t("ajustes.unidadesPeso")}
              </p>
              <p className="mt-0.5 text-xs text-muted">
                {t("ajustes.unidadesPesoDesc")}
              </p>
            </div>
            <Select
              ...
              onChange={(v) => {
                ...
                void update({
                  units,
                  measurementSystem: units === "lb" ? "imperial" : "metric",
                });
              }}
            ...
            <div className="space-y-3 rounded-xl pt-3 border-t border-border/30">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="text-xs text-muted">
                  {t("ajustes.seriesPrecargarHint")}
                </p>
                <NumberField
                  value={settings.preloadSetCount}
                  onChange={(v) =>
                    void update({ preloadSetCount: Math.max(0, v || 0) })
                  }
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  <Select
                    ...
                    onChange={(v) =>
                      void update({ preloadWeightMode: v as PreloadWeightMode })
                    }
                  ...
                  {settings.preloadWeightMode !== "exact" && (
                    <NumberField
                      ...
                      onChange={(v) =>
                        void update({ preloadWeightValue: Math.max(0, v || 0) })
                      }
                      label={t("ajustes.ajustePeso")}
                      min={0}
                      max={100}
          ...
          {settings.showLoadSuggestion && (
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl pt-3 border-t border-border/30">
              ...
              <NumberField
                ...
                onChange={(v) =>
                  void update({
                    loadProgressionPct: Math.max(0.5, Math.min(10, v || 2.5)),
                  })
                }
        ...
        {/* --- Sección: Notificaciones (recordatorios push) */}
        <section className="panel-light rounded-2xl p-4">
          <div className="flex items-center gap-2">
            ...
            <>
              {notif.permission === "denied" && (
                <p className="text-xs text-danger">
                  {t("ajustes.notificacionesPermisoDenegado")}
                </p>
              )}
              <div className="flex items-center justify-between gap-3 border-t border-border/50 pt-3">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-fg">
                    {t("ajustes.recordatorioEntreno")}
                  </p>
                  <p className="mt-0.5 text-xs text-muted">
                    {t("ajustes.recordatorioHora")}
                  </p>
                </div>
                <input
                  type="time"
                  ...
                  onChange={(e) => {
                    ...
                    void update({
                      trainingReminderHour: h ?? 18,
                      trainingReminderMinute: m ?? 0,
                    });
                  }}
              ...
              <Toggle
                ...
                onChange={(v) => void update({ streakReminder: v })}
                label={t("ajustes.recordatorioRacha")}
                description={t("ajustes.recordatorioRachaDesc")}
              />
              <Toggle
                checked={settings.inactivityReminder}
                onChange={(v) => void update({ inactivityReminder: v })}
                label={t("ajustes.recordatorioInactividad")}
                description={t("ajustes.recordatorioInactividadDesc")}
              />
            </>
          )}
        </section>
        ...
        <section className="panel-light rounded-2xl p-4">
          ...
          <div className="flex items-center justify-between gap-3 border-t border-border/50 pt-3">
            <div className="min-w-0">
              <p className="text-sm font-medium text-fg">
                {t("ajustes.undoDelete")}
              </p>
              <p className="mt-0.5 text-xs text-muted">
                {t("ajustes.undoDeleteDesc")}
              </p>
            </div>
            <NumberField
              value={settings.undoDurationSec}
              onChange={(v) =>
                void update({ undoDurationSec: Math.max(0, v || 0) })
              }
        ...
        <section className="panel-light rounded-2xl p-4">
          ...
          {showImport && (
            <div className="mt-2 space-y-3 rounded-xl pt-3 border-t border-border/30">
              ...
              {importMessage && (
                <p className="text-xs text-accent-soft">{importMessage}</p>
              )}
            </div>
          )}
          {showBackup && (
