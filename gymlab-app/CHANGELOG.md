...
### Changed
- **Rediseño del calendario semanal de Entrenar (`UX`, skill frontend-design)**: la semana vigente pasa a una "raíl" industrial-premium que encaja con el tema de la app: pista oscura con 7 celdas con barra de estado LED superior (verde = hecho, CTA = programado, gris = descanso), día en Oswald grande, badge `D1..Dn` del día de rutina, check para los entrenados y contador "X/7 días"; hoy se marca con "Hoy", número en acento y glow dorado (`gold-border-glow`), y las celdas entran con reveal escalonado (respetando `prefers-reduced-motion`). Adaptado a las 5 paletas vía tokens (`WeekCalendar.tsx`). Verificado con Playwright.
