...
### Changed
- **Calendario semanal en Entrenar (`UX`)**: la semana vigente se muestra como una fila limpia de 7 días (letra del día + número en Oswald) con la fecha de hoy en la cabecera («6 agosto») y los días entrenados con el número en verde; todo el bloque es un enlace que lleva al calendario completo. Se elimina el título "Calendario" y el botón "Ver completo" de la home (la información extra queda en `/calendario`). Adaptado a las 5 paletas vía tokens (`WeekCalendar.tsx`). Verificado con Playwright.
