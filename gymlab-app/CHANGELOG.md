...
### Changed
- **Calendario semanal minimalista en Entrenar (`UX`)**: la semana vigente se muestra como una fila limpia de 7 días (letra del día + número en Oswald, separados con `justify-between`), con un punto verde bajo los días entrenados y un círculo dorado para el día de hoy; el resto de la información (días programados, contador, leyenda) se deja para el calendario completo (`/calendario`). Adaptado a las 5 paletas vía tokens (`WeekCalendar.tsx`). Verificado con Playwright.
