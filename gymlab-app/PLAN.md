...
### [x] Fase 68 — Retos dinámicos adaptativos
...
- [x] Recompensa: logro «Primera meta» (`primer-reto`) al completar un reto ← el conteo es **histórico** (`countEverCompletedChallenges`, dominio puro): el progreso del reto se resetea por periodo, el logro no. Entra por el pipeline normal de `checkAchievements` → `meta.unlockedAchievements`, así que se ve en `/logros` con su barra. El pill «¡Completado!» (`challenge.done`) de la tarjeta sigue siendo efímero **a propósito**: es feedback del periodo, no el premio
- [x] Keys es/en + `tsc` + build + tests + CHANGELOG + commit
...
> **Recompensa cerrada (2026-09-18)**: `countEverCompletedChallenges()` (conteo histórico de retos completados, dominio puro) + logro `primer-reto` (bronce) con la medida `completedChallengeCount`; el catálogo de logros pasa de 15 a 16 ids. De paso se corrigió un defecto latente: el `now` de `computeChallengeStats` no llegaba a `isInPeriod`/`workoutsInPeriod`/`prsInPeriod`/`setsInPeriod`, así que **toda** evaluación histórica quedaba anclada a hoy — lo destapó el test rojo del conteo histórico. Verificación: `npm test` **83 files / 949 tests**, `npm run build` ✓ 9.58s, e2e `test_f93_16_chapas.py` **ALL OK**; los contadores de los e2e pasan a `/16`. **Review**: el usuario **declinó** el ciclo para este candidato (`declined_this_candidate`, sin linaje ni recibo) — se commitea sin review por decisión explícita suya.
>
> **Hallazgo aparte, NO de F68** (detectado al correr los e2e): `tests/e2e/test_f95.py` está **rojo** porque su seed siembra `equipment: 'maquina'` / `'barra'` como **string** contra el contrato de array que dejó F66/F67 → `TypeError: equipment.map is not a function` en `PickerRow` (`ExercisePicker.tsx`). Está en HEAD y no lo tocó este cambio; hay que actualizar el seed de esos e2e (2 líneas).

### [x] Fase 69 — Comparación de sesiones (rediseño)
- [x] `SessionComparison.tsx` + vista lado a lado + deltas
- [x] Integrado en historial
- [x] Más métricas comparables: ejercicios, series, reps, PRs logrados, ejercicios nuevos, grupos musculares trabajados, intensidad media (volumen/reps), peso medio por serie y e1RM medio
- [x] Calorías por sesión ← **estimadas en dominio puro** desde datos persistidos: MET por ejercicio (`metForSlug`, movido a `domain/cardio.ts`) × `durationSeconds` × peso corporal sobre las series de trabajo temporizadas; **sin migración** (se recalcula) y «—» cuando falta peso o duración
- [x] Keys es/en + `tsc` + build + tests + CHANGELOG + commit

> **Rediseño cerrado con ciclo de review completo (2026-09-17)**, en worktree aislado (`gymlab-f69`, rama `f69-comparativa-sesiones`). Corrige los **tres problemas verificados** que motivaron el rediseño: **deltas invertidos** (el componente hacía `vieja − nueva` sobre una lista que llega más reciente primero: mejorar mostraba caída en rojo), **piso tipográfico** (`text-[0.55rem]` ≈ 8,8 px) y **a11y/tokens/touch** (grid de `<p>` sin semántica de tabla, selectores sin label accesible ni 44 px, `text-red-400` crudo). Ahora ordena **anterior → posterior** con `delta = posterior − anterior` en dominio puro nuevo (`src/domain/sessionComparison.ts`) y muestra **11 métricas** en tres bloques con signo explícito, más barras por grupo muscular. Verificación: `npm run build` limpio (`tsc -b` real), `npm test` **908 tests / 82 archivos** y e2e `tests/e2e/test_f69.py` **ALL OK**. **Con ciclo de review (RDD)**, lineage `review-e263f790da52d942`: reviewer + refuter + una corrección acotada + validador targeted → **APROBADO** (receipt quemado). El refuter **corroboró un bug real introducido por el propio rediseño** — `MuscleGroupBars` pintaba el volumen crudo en kg con la etiqueta de la unidad elegida, sin pasar por `applyUnits` (mentía en libras) — ya corregido. Advisory no bloqueante: `R3-002` (SUGGESTION) en `tests/e2e/test_f69.py:90`.
