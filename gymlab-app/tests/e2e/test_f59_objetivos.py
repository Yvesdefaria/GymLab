...
def assert_proyeccion(page, view_name, shot_prefix):
    ...
    expect(page.get_by_text(f"~75 semanas ({expected_date})", exact=True)).to_be_visible()
    expect(page.get_by_text("+0.5 kg/semana", exact=True)).to_be_visible()

    page.screenshot(path=f"{shot_prefix}-proyeccion.png", full_page=False)
    ...
    page.screenshot(path=f"{shot_prefix}-alcanzado.png", full_page=False)
    ...
    page.screenshot(path=f"{shot_prefix}-sin-datos.png", full_page=False)
